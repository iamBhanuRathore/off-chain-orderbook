...
export function OrderBookPanel({ selectedSymbol }: OrderBookPanelProps) {
  const { bids: bidsFromHook, asks: asksFromHook } = useOrderBookSocket(selectedSymbol);
  ...
  const { processedAsks, processedBids, maxCumulativeAsk, maxCumulativeBid, lowestAskPrice, highestBidPrice, totalBidQuantity, totalAskQuantity } = useMemo(() => {
    ...
    const asksWithCumulative = asksFromHook
      .slice(0, 12) 
      .map(ask => {
    ...
    const bidsWithCumulative = bidsFromHook
      .slice(0, 12)
      .map(bid => {
    ...
    <Card className="flex-1 flex flex-col h-full bg-card text-foreground">
      <Tabs defaultValue="orderbook" className="w-full flex-grow flex flex-col">
        <CardHeader className="p-0">
          <TabsList className="grid w-full grid-cols-2 rounded-none h-10 bg-card border-b border-border">
            ...
            <TabsTrigger 
              ...
              className="py-1 text-xs rounded-none data-[state=active]:shadow-none data-[state=active]:bg-muted data-[state=active]:text-foreground focus-visible:ring-0 focus-visible:ring-offset-0"
            >
              <History size={14} className="mr-2"/>Trades
            </TabsTrigger>
          </TabsList>
        </CardHeader>
        ...
        <TabsContent 
          value="orderbook" 
          className={cn(
            "flex-grow flex flex-col p-0 m-0 overflow-hidden", 
            "data-[state=inactive]:hidden" 
          )}
        >
          <div className="flex justify-between items-center py-1.5 px-2 border-b border-border">
            <div className="flex items-center gap-0.5">
              <Button variant="ghost" size="icon" className={cn("h-6 w-6 text-muted-foreground hover:text-foreground", bookView === 'combined' && "bg-muted text-foreground")} onClick={() => setBookView("combined")}>
                <ListTree size={14}/>
              </Button>
              <Button variant="ghost" size="icon" className={cn("h-6 w-6 text-muted-foreground hover:text-foreground", bookView === 'bids' && "bg-muted text-foreground")} onClick={() => setBookView("bids")}>
                <ArrowDownWideNarrow size={14}/>
              </Button>
               <Button variant="ghost" size="icon" className={cn("h-6 w-6 text-muted-foreground hover:text-foreground", bookView === 'asks' && "bg-muted text-foreground")} onClick={() => setBookView("asks")}>
            ...
            <div className="flex items-center gap-1">
                ...
                <Button variant="ghost" size="icon" className="h-6 w-6 text-muted-foreground hover:text-foreground" onClick={handleIncreaseAggregation} disabled={currentAggIndex === aggregationLevels.length - 1}>
                    <Plus size={12}/>
                </Button>
            </div>
          </div>
          ...
          <div className="py-1.5 px-3 text-center border-y border-border bg-card">
            {midPrice !== undefined ? (
                <span className="text-lg font-medium text-green-500 tabular-nums"> 
                  {midPrice.toFixed(2)}
                </span>
            ) : (
          ...
          <div className="px-3 py-2 border-t border-border">
             <div className="h-3 bg-muted rounded-sm flex text-xs items-center relative">
              <div 
                className="bg-green-500/80 h-full flex items-center justify-center text-white font-medium rounded-l-sm" 
                style={{width: `${bidDepthPercentage}%`}}
              >
                {bidDepthPercentage > 10 && <span className="text-[10px] leading-none">{Math.round(bidDepthPercentage)}%</span>}
              </div>
              <div 
                className="bg-destructive/80 h-full flex items-center justify-center text-white font-medium rounded-r-sm" 
                style={{width: `${askDepthPercentage}%`}}
              >
