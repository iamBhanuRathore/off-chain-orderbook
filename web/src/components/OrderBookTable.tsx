      ...
      <Table className="relative">
        <TableHeader className="sticky top-0 bg-card z-10">
          <TableRow className="border-none text-xs">
            <TableHead className="w-1/3 h-10 p-1 text-left text-muted-foreground font-normal">Price ({quoteAsset})</TableHead>
            <TableHead className="w-1/3 h-10 text-center p-1 text-muted-foreground font-normal">Size ({baseAsset})</TableHead>
            <TableHead className="w-1/3 h-10 text-right p-1 text-muted-foreground font-normal">Total ({baseAsset})</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          <AnimatePresence initial={false}>
            {data.map((entry, index) => {
              ...
              return (
                ...
                >
                  <TableCell className={cn("py-0.5 px-2 tabular-nums text-left w-1/3 font-mono", isBids ? "text-green-500" : "text-red-500")}>{entry.price.toFixed(pricePrecision)}</TableCell>
                  <TableCell className="py-0.5 px-2 text-right tabular-nums w-1/3 font-mono text-foreground/90">{entry.quantity.toFixed(quantityPrecision)}</TableCell>
                  <TableCell className="py-0.5 px-2 text-right tabular-nums relative w-1/3 font-mono text-foreground/70">
                    <div
                      className={cn(
                        "absolute top-0 bottom-0 right-0 h-full opacity-40", // Changed to right-0
